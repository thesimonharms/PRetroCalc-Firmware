...
static void game_over(const char *msg, int score) {
    char buf[40];
    gfx_fill_rect(40, 130, 240, 60, COL_BLACK);
    gfx_rect(40, 130, 240, 60, COL_RED);
    gfx_puts_at(60, 142, "GAME OVER", COL_RED, COL_BLACK);
    snprintf(buf, sizeof buf, "%s  SCORE:%d", msg, score);
    gfx_puts_at(52, 160, buf, COL_WHITE, COL_BLACK);
    gfx_puts_at(52, 176, "ENTER=again  ESC=menu", COL_GRAY, COL_BLACK);
    gfx_flush();
}
...
#define SN_CELL 8
#define SN_W (LCD_WIDTH / SN_CELL)
#define SN_H (LCD_HEIGHT / SN_CELL)
#define SN_MAX 256
...
restart:;
    ...
    int ax = 20, ay = 10, score = 0;
    int speed = 130;
    game_window("SNAKE");
    for (;;) {
        gfx_fill_rect(pw_x, pw_y, LCD_WIDTH - 2 * pw_x, LCD_HEIGHT - pw_y - 10, COL_BLACK);
        /* food */
        gfx_fill_rect(ax * SN_CELL + 1, ay * SN_CELL + 1, SN_CELL - 2, SN_CELL - 2, COL_RED);
        /* snake */
        for (int i = 0; i < len; i++)
            gfx_fill_rect(sx[i] * SN_CELL + 1, sy[i] * SN_CELL + 1, SN_CELL - 2, SN_CELL - 2,
                          i == 0 ? COL_LGREEN : COL_GREEN);
        char b[24]; snprintf(b, sizeof b, "SCORE %d", score);
        gfx_puts_at(2, 2, b, COL_WHITE, COL_BLACK);
        gfx_flush();
...
restart:;
    ...
    int paddle_w = 48, px = (LCD_WIDTH - paddle_w) / 2;
    int bx = LCD_WIDTH/2, by = 200, bdx = 2, bdy = -3;
    int score = 0, lives = 3;
    ...
    for (;;) {
        ...
        for (int r = 0; r < rows; r++)
            for (int c = 0; c < cols; c++)
                if (bricks[r][c])
                    gfx_fill_rect(c * 32 + 1, 20 + r * 12, 30, 10, rowcol[r]);
        gfx_fill_rect(px, LCD_HEIGHT - 16, paddle_w, 6, COL_WHITE);
        ...
        poll_key(&c, 14);
        if (c == KEY_ESC) return;
        /* hold-friendly: also check current modifiers via events */
        if (c == KEY_LEFT || c == 'a') { px -= 8; if (px < 0) px = 0; }
        if (c == KEY_RIGHT || c == 'd') { px += 8; if (px > LCD_WIDTH - paddle_w) px = LCD_WIDTH - paddle_w; }
        /* auto-move with held keys: sample more events quickly */
        int c2;
        while (poll_key(&c2, 1)) {
            if (c2 == KEY_LEFT) { px -= 8; if (px < 0) px = 0; }
            if (c2 == KEY_RIGHT) { px += 8; if (px > LCD_WIDTH - paddle_w) px = LCD_WIDTH - paddle_w; }
            if (c2 == KEY_ESC) return;
        }

        bx += bdx; by += bdy;
        if (bx < 3 || bx > LCD_WIDTH - 3) { bdx = -bdx; sound_beep(800, 10); }
        if (by < 12) { bdy = -bdy; sound_beep(800, 10); }
        if (by > LCD_HEIGHT - 22 && by < LCD_HEIGHT - 10 && bx >= px && bx <= px + paddle_w && bdy > 0) {
            bdy = -bdy;
        ...
        if (by > LCD_HEIGHT) {
            ...
            if (lives == 0) break;
            bx = LCD_WIDTH/2; by = 200; bdx = 2; bdy = -3;
        }
        /* brick collision */
        if (bdy < 0 && by >= 20 && by < 20 + rows * 12) {
            int r = (by - 20) / 12, cc = bx / 32;
        ...
        if (left == 0) {
            gfx_puts_at(100, 150, "YOU WIN!", COL_LGREEN, COL_BLACK);
            gfx_flush();
            poll_key(&c, 100000);
            if (c == KEY_ESC) return;
            goto restart;
...
restart:;
    ...
    int inv_x = 20, inv_y = 30, inv_dx = 2;
    int ship_x = LCD_WIDTH / 2;
    int bul_x = -1, bul_y = -1;
    ...
    for (;;) {
        ...
        for (int r = 0; r < INV_ROWS; r++)
            for (int c = 0; c < INV_COLS; c++)
                if (inv[r][c]) {
                    int x = inv_x + c * 34, y = inv_y + r * 22;
                    gfx_fill_rect(x + 2, y + 4, 20, 10, COL_MAGENTA);
                    gfx_fill_rect(x, y + 8, 24, 4, COL_MAGENTA);
                    alive++;
                }
        /* ship + bullets */
        gfx_fill_rect(ship_x - 12, LCD_HEIGHT - 20, 24, 6, COL_LGREEN);
        gfx_fill_rect(ship_x - 2, LCD_HEIGHT - 26, 4, 8, COL_LGREEN);
        ...
        poll_key(&c, 30);
        if (c == KEY_ESC) return;
        if (c == KEY_LEFT) { ship_x -= 6; if (ship_x < 14) ship_x = 14; }
        if (c == KEY_RIGHT) { ship_x += 6; if (ship_x > LCD_WIDTH - 14) ship_x = LCD_WIDTH - 14; }
        if ((c == ' ' || c == KEY_ENTER) && bul_y < 0) { bul_x = ship_x; bul_y = LCD_HEIGHT - 30; sound_beep(1600, 20); }

        /* move swarm */
        inv_x += inv_dx;
        if (inv_x < 4 || inv_x + INV_COLS * 34 > LCD_WIDTH - 8) {
            inv_dx = -inv_dx;
            inv_y += 10;
            if (inv_y + INV_ROWS * 22 > LCD_HEIGHT - 50) break; /* invaded */
        }
        /* enemy fire */
        if (ebul_y < 0 && (rand() % 30) == 0 && alive > 0) {
            int r, cc, tries = 20;
            do { r = rand() % INV_ROWS; cc = rand() % INV_COLS; } while (!inv[r][cc] && --tries);
            ebul_x = inv_x + cc * 34 + 12; ebul_y = inv_y + r * 22 + 16;
        }
        if (bul_y >= 0) {
            bul_y -= 6;
            if (bul_y < 0) bul_y = -1;
                ...
                for (int r = 0; r < INV_ROWS && bul_y >= 0; r++)
                    for (int c2 = 0; c2 < INV_COLS; c2++)
                        if (inv[r][c2]) {
                            int x = inv_x + c2 * 34, y = inv_y + r * 22;
        ...
        if (ebul_y >= 0) {
            ebul_y += 5;
            if (ebul_y > LCD_HEIGHT) ebul_y = -1;
            else if (ebul_y > LCD_HEIGHT - 28 && ebul_x > ship_x - 14 && ebul_x < ship_x + 14) {
...
#define LIFE_CELL 4
#define LIFE_W (LCD_WIDTH / LIFE_CELL)
#define LIFE_H (LCD_HEIGHT / LIFE_CELL)
        ...
        for (int y = 0; y < LIFE_H; y++)
            for (int x = 0; x < LIFE_W; x++)
                if (life_a[y*LIFE_W+x])
                    gfx_fill_rect(x*LIFE_CELL, y*LIFE_CELL, LIFE_CELL-1, LIFE_CELL-1, COL_GREEN);
        char b[32]; snprintf(b, sizeof b, "GEN %d  %s", gen, running ? "RUN " : "STOP");
        gfx_puts_at(2, 2, b, COL_WHITE, COL_BLACK);
        gfx_puts_at(2, LCD_HEIGHT-10, "SPACE=run/stop R=rand ESC=exit", COL_GRAY, COL_BLACK);
        gfx_flush();
